#!/bin/bash
# AWS Monitor Configuration Manager
...
CONFIG_FILE="${MONITOR_CONFIG:-$PROJECT_DIR/config.yaml}"
...
def cmd_list(config_path):
    ...
    print("\n\033[1;36m═══════════════════════════════════════════\033[0m")
    print("\033[1;36m  AWS Monitor - Configured Services\033[0m")
...
usage() {
    echo -e "${CYAN}AWS Monitor Configuration Manager${NC}"
    echo ""
    ...
    echo "  MONITOR_CONFIG    Path to config.yaml (default: ../config.yaml)"
    echo ""
}
